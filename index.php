...
/* ================= URL HELPERS ================= */
function isHttpsReq() {
  if (!empty($_SERVER["HTTP_X_FORWARDED_PROTO"])) return strtolower($_SERVER["HTTP_X_FORWARDED_PROTO"]) === "https";
  if (!empty($_SERVER["HTTPS"]) && $_SERVER["HTTPS"] !== "off") return true;
  return false;
}
function baseUrlThisFile() {
  $proto = isHttpsReq() ? "https" : "http";
...
function tg($method, $data = []) {
  ...
  $res = curl_exec($ch);
  if ($res === false) error_log("TG CURL ERROR: ".curl_error($ch));
  curl_close($ch);
...
/* ===== Old-PHP compatible SameSite cookie ===== */
...
/* ✅ Ensure user exists before token update */
function makeVerifyLink($tg_id) {
  global $pdo;
  ensureUserExists($tg_id);
  ...
  try {
    ...
    error_log("VERIFY TOKEN UPDATE ERROR: ".$e->getMessage());
  }
...
if ($_SERVER["REQUEST_METHOD"] === "GET") {
  ...
  try {
    ...
    error_log("VERIFY SELECT ERROR: ".$e->getMessage());
    echo htmlVerifyUI("DB Error", "Missing verify columns. Run the provided SQL.", null);
    exit;
  }
  ...
  try {
    ...
    error_log("DEVICE_LINKS ERROR: ".$e->getMessage());
    echo htmlVerifyUI("DB Error", "device_links table missing. Run the provided SQL.", null);
    exit;
  }
  ...
  try {
    ...
    error_log("VERIFY UPDATE ERROR: ".$e->getMessage());
    echo htmlVerifyUI("DB Error", "Could not update verified status.", null);
    exit;
  }
...
if (isset($update["message"])) {
        ...
        if ($refUser) {
          ...
          try {
            $pdo->prepare("UPDATE users
                           ...
                           WHERE tg_id = :r")->execute([":r" => $referred_by]);
          } catch (Exception $e) {}
        }
      }
  ...
  // ---- if not verified, show join buttons ----
  if (!isVerifiedUser($from_id)) {
  ...
  if ($text === "📊 Stats") {
    ...
    sendMessage($chat_id, "📊 <b>Your Stats</b>\n\n⭐ Points: <b>{$points}</b>\n👥 Referrals: <b>{$refs}</b>", userReplyKeyboard(isAdmin($from_id)));
    http_response_code(200); echo "OK"; exit;
  }
...
if (isset($update["callback_query"])) {
  ...
  if ($data === "check_join") {
    if (allJoined($from_id)) {
      ...
      sendMessage($chat_id, "🔐 <b>Verification</b>\nTap below to verify.", verifyMenuMarkup($url));
    } else {
